### General packages
brew install ack ctags diff-so-fancy gpg ffmpeg fpp gettext gnu-sed imagemagick puma/puma/puma-dev rename ssh-copy-id tmux vim wget xz

### General packages - Casks
brew cask install chromedriver

### Package managers

# Use asdf to replace specific language version managers:
# brew install nvm pyenv rbenv
brew install asdf

# Dependent packages for asdf
brew install coreutils automake autoconf openssl libyaml readline libxslt libtool unixodbc unzip curl
